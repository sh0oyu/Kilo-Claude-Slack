...
const Game = {
    ...
    buyBuilding(buildingKey) {
        ...
        if (!this.canAfford(costs)) {
            ...
            const card = document.querySelector(`.building-card[data-building="${buildingKey}"]`);
            if (card) {
        ...
        if (card) {
            ...
            setTimeout(() => card.classList.remove('purchase-flash'), 500);
        }
    ...
    craft(recipeKey) {
        ...
        // Flash the craft card
        const card = document.querySelector(`.craft-card[data-recipe="${recipeKey}"]`);
        if (card) {
            card.classList.remove('purchase-flash');
            void card.offsetWidth;
            card.classList.add('purchase-flash');
    ...
    updateUI() {
        ...
        // Production pulse on resource items with positive rate
        for (const key in RESOURCES) {
            if (RESOURCES[key].tier >= 4) continue;
            const rate = this.getProductionRate(key);
            const item = document.querySelector(`.resource-item[data-resource="${key}"]`);
            if (item) {
                if (rate > 0) {
                    item.classList.add('producing');
                } else {
                    item.classList.remove('producing');
                }
            }
        }
...
function createClickFeedback(event, text) {
    ...
    el.textContent = text;
    const x = (event.clientX || event.pageX || 100) + (Math.random() - 0.5) * 30;
    const y = (event.clientY || event.pageY || 100) - 10;
    el.style.left = x + 'px';
    el.style.top = y + 'px';
...
function updateEventIndicator() {
    const container = document.getElementById('event-indicator');
    if (!container) return;
    container.innerHTML = '';
    ...
    for (const evt of Game.activeEvents) {
        ...
        badge.innerHTML = `
            <span>${evt.icon}</span>
            <span>${evt.name}</span>
            <span class="event-badge-timer">${Math.ceil(evt.remaining)}s</span>
        `;
